...
> 这个分支（`claude/react-rewrite`）在原来这套纯静态站点（`docs/`，本 README 剩下的部分都是讲它）之外，
> 新增了一套 React 版前端 + 一个小后端服务：
> - [`web/`](web) —— 界面重写成 React（Vite 打包），数据层原理不变，还是直连 Supabase、靠 RLS 保证隔离
> - [`server/`](server) —— 只做一件静态站点做不到的事：定时账单真正按服务器时间表自动生成，不用等谁打开网站
>
> 两套前端（`docs/` 和 `web/`）共用同一个 Supabase 项目和同一份 [`sql/schema.sql`](sql/schema.sql)，可以只部署其中一套，也可以两个都留着对照。具体怎么开发/部署看各自目录下的 README。
